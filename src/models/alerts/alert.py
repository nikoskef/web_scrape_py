...
class Alert:
    ...
    def __repr__(self):
        return f'<Alert for {self.user_email} on item {self.item.name} with price {self.price_limit}>'
    ...
    def send(self):
        return requests.post(
            ...
            data={
                ...
                "text": "We've found a deal! ({}) To navigate to the alert, visit http://pricing...".format(
                    self.item.url, "http://pricing...")
            }
        )
    ...
    def find_needing_update(cls, minutes_since_update=AlertConstants.ALERT_TIMEOUT):
        ...
        return [cls(**elem) for elem in Database.find(AlertConstants.COLLECTION,
                                                      {"last_checked":
                                                            {"$lte": last_updated_limit},  # less than equal
                                                       "active": True
                                                       })]
